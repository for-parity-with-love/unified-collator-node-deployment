...
3) Open tfvars file located in tfvars directory and edit these variables
```
aws_region                          = "eu-central-1"
aws_profile_name                    = "blaize" 
```
5) Rename `backend.tmp` to `backend.tf` and edit `bucket`, `region`, `profile` values inside.
6) Edit `tfvarf/*.tfars` and change also all needed values
